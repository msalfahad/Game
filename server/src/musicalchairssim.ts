...
export class MusicalChairsSim implements GameSim {
  ...
  private gapT = 0;
  private outCount = 0;
  private wantAct = new Set<string>(); // socketIds that pressed this tick
  ...
  applyInput(socketId: string, msg: InputMsg) {
    if (msg.ult || msg.jump) this.wantAct.add(socketId);
  }
  ...
  private startRound() {
    ...
    alive.forEach((p, i) => {
      p.sitting = false; p.seat = null; p.runBoost = 0;
  ...
  private stopSong() {
    ...
    this.events.push({ t: 'out', slot: -1 }); // SIT! cue (slot -1 = no player)
  }

  private claimSeat(p: MPlayer): boolean {
    if (p.seat != null || p.dead) return false;
    ...
    p.seat = best;
    this.events.push({ t: 'power', slot: p.slot }); // seat-claim cue
    return true;
  }
  ...
  private step(dt: number) {
    ...
    if (this.phase === PHASE.walk) {
      ...
      for (const p of this.alive()) {
        ...
        // RUN bursts: humans on press, bots occasionally.
  ...
  private sendState() {
    const players: PlayerState[] = this.players.map((p) => [
      ...
      p.dead ? 0 : 1, p.dead ? 1 : 0, 0, 0, 0, 0, p.sitting ? 1 : 0,
    ]);
