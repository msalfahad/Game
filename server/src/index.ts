...
io.on('connection', (socket) => {
  ...
  socket.on('room:serieslen', guard1<number>((len) => lobby.setRoomLen(socket.id, Number(len))));
  socket.on('room:gametoggle', guard1<string>((gameId) => lobby.toggleRoomGame(socket.id, String(gameId))));
