...
// mechanics the server can simulate. The bespoke offline-only games (boat, raft,
// foosball, sprint, …) are excluded so the picker never offers a game the
// server can't run.
const ONLINE_MECHANICS = new Set(['goal', 'icepush', 'climb', 'breaktiles', 'pushout', 'throwfight', 'dodge', 'collect', 'paint', 'mash', 'hotpotato', 'musicalchairs', 'chase', 'kart', 'maze', 'foosball']);
